...
class SynthSoundEngine {
  ...
  // 射击音效 - 能量武器发射声
  playShoot(volume = 0.4) {
    if (!this.initialized || !this.noiseSynths || !this.generalSynths) return
    ...
    noise.volume.value = Tone.gainToDb(volume * 0.45)
    noise.triggerAttackRelease('32n')
    ...
    synth.frequency.exponentialRampTo('A2', 0.1, now)
    synth.triggerRelease(now + 0.12)
  }

  // 换弹音效 - 机械咔嗒声
  playReload(volume = 0.3) {
    if (!this.initialized || !this.clickSynths || !this.noiseSynths) return

    const click = this.getFromPool(this.clickSynths)
    click.volume.value = Tone.gainToDb(volume * 0.35)
    const now = Tone.now()

    // 弹匣取出
    click.triggerAttack('C4', now)
    click.frequency.exponentialRampTo('G3', 0.03, now)
    click.triggerRelease(now + 0.04)

    // 弹匣装入
    click.triggerAttack('E4', now + 0.5)
    click.triggerRelease(now + 0.55)

    // 上膛
    click.triggerAttack('G4', now + 1.0)
    click.frequency.exponentialRampTo('C5', 0.05, now + 1.0)
    click.triggerRelease(now + 1.08)
    ...
    noise.triggerAttackRelease('64n', now + 0.5)
    noise.triggerAttackRelease('64n', now + 1.0)
  }
