...
class SoundManager {
  ...
  playShoot() {
    const store = useAudioStore.getState()
    if (store.muted) return
    synthSounds.playShoot(store.getEffectiveSfxVolume())
  }

  playReload() {
    const store = useAudioStore.getState()
    if (store.muted) return
    synthSounds.playReload(store.getEffectiveSfxVolume())
  }

  playFootstep() {
    const store = useAudioStore.getState()
    if (store.muted) return
    synthSounds.playFootstep(store.getEffectiveSfxVolume())
  }
...
export function useGameSound() {
  return {
    ...
    playShoot: () => soundManager.playShoot(),
    playReload: () => soundManager.playReload(),
    playFootstep: () => soundManager.playFootstep(),
